...
class DQNAgent:
    ...
    def act(self, state):
        if np.random.rand() <= self.epsilon:
            return random.randrange(self.action_size) + 11
        state = torch.FloatTensor(state).unsqueeze(0)
        act_values = self.model(state)
        return torch.argmax(act_values[0]).item() + 11
...
def train_dqn_agents(k=5, episodes=1000, batch_size=32):
    ...
    action_size = 10  # Actions are from 11 to 20, hence 10 possible actions
    ...
    plt.ylabel("Agent 1 Choices (11 to 20)")
    plt.colorbar(ticks=range(11, 21), label='Choice')
    ...
    plt.xlabel("Time Steps")
    plt.ylabel("Agent 2 Choices (11 to 20)")
    plt.colorbar(ticks=range(11, 21), label='Choice')
