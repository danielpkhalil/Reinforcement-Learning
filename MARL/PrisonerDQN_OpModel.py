...
class DQNAgent:
    def __init__(self, state_size, action_size, hidden_size=64):
        ...
        # Opponent model
        self.opponent_model = self._build_model()
    ...
    def act(self, state, opponent_action):
    ...
    def act_opponent(self, state):
        state = torch.FloatTensor(state).unsqueeze(0)
        act_values = self.opponent_model(state)
        return torch.argmax(act_values[0]).item()
...
def train_dqn_agents(k=5, episodes=1000, batch_size=32):
    ...
    for e in range(episodes):
        ...
        for time in range(200):
            action1 = agent1.act(state, None)
            predicted_action1 = agent2.act_opponent(state)  # Agent 2 predicts Agent 1's action
            action2 = agent2.act(state, predicted_action1)  # Agent 2 best responds to predicted action
            ...
            if done:
                ...
                agent2.update_opponent_model()  # Update Agent 2's opponent model
                break
